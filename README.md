...
Not only in the physics department it is good practice to use the [SI format](https://en.wikipedia.org/wiki/International_System_of_Units) for writing down any number (especially very large or small numbers). This ensures easy readability (thanks to the SI prefixes as n, µ, m, k, M, G, ...) and only produces an output as precise as necessary (usually 3 digits are the sweet spot, what means the total number of meaningful digits like 56.9). 
