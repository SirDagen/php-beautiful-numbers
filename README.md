...
*Ann.: We use an array for the language part so that it is easier to implement* php-beautiful-numbers *in multi-language websites, e.g. tnum($val, $LANG['de']['baum-AKK']) for the German accusative form like ["Bäume", "einen Baum"] or tnum($val, $LANG['de']['baum-NOM']) for the German nominative form like ["Bäume", "ein Baum"].*
